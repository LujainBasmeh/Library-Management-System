...
public class PatronControllerTest {
	...
	public void testFindById_failure() throws Exception {
		...
		// Act and Assert
		mockMvc.perform(get("/api/patrons/2"))
	...
	public void testUpdateById_success() throws Exception {
		...
		Patron updatedPatron = new Patron(id, "User 1", "user1@gmail.com", "111222", "new address1");
		when(patronService.updateById(id, updatedPatron)).thenReturn(updatedPatron);
		// Act and Assert
		mockMvc.perform(put("/api/patrons/" + id)
				.contentType(MediaType.APPLICATION_JSON)
				.content(
				"{\"name\":\"User 1\",\"email\":\"user1@gmail.com\",\"phone\":\"111222\",\"address\":\"new address1\"}"))
				.andExpect(status().isOk());
	...
	public void testUpdateById_failure() throws Exception {
		...
		mockMvc.perform(put("/api/patrons/" + id)
				...
				"{\"name\":\"User 1\",\"email\":\"user1@gmail.com\",\"phone\":\"111222\",\"address\":\"new address1\"}"))
				.andExpect(status().isNotFound())
				.andExpect(result -> assertEquals("Patron with id: " + id + " not found",
	...
	public void testDeleteById_failure() throws Exception {
		// Arrange
		long id = 99L;
		doThrow(new RuntimeException("Patron with id: " + id + " not found")).when(patronService).deleteById(id);
